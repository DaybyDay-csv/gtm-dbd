...
serve(async (req) => {
  ...
  try {
    ...
    const { projectId, allPhaseData, budgetLevel, budgetAmount, channelPreference, outputLanguage } = inputSchema.parse(body);
...
- El disclosure debe ser un párrafo coherente de 3-4 líneas explicando la lógica estratégica`;
    ...
    if (!validation.success) {
      console.warn('phase-6-channel-strategy output schema validation (non-fatal):', JSON.stringify(validation.error.issues).slice(0, 500));
    }
