...
const TONES = [{
  value: 'professional',
  labelKey: 'hero.tone.professional'
}, {
  value: 'friendly',
  labelKey: 'hero.tone.friendly'
}, {
  value: 'technical',
  labelKey: 'hero.tone.technical'
}, {
  value: 'inspirational',
  labelKey: 'hero.tone.inspirational'
}, {
  value: 'direct',
  labelKey: 'hero.tone.direct'
}, {
  value: 'empathetic',
  labelKey: 'hero.tone.empathetic'
}];
const INDUSTRIES = [{
  value: 'saas',
  labelKey: 'hero.industry.saas'
}, {
  value: 'ecommerce',
  labelKey: 'hero.industry.ecommerce'
}, {
  value: 'services',
  labelKey: 'hero.industry.services'
}, {
  value: 'education',
  labelKey: 'hero.industry.education'
}, {
  value: 'healthcare',
  labelKey: 'hero.industry.healthcare'
}, {
  value: 'finance',
  labelKey: 'hero.industry.finance'
}, {
  value: 'other',
  labelKey: 'hero.industry.other'
}];
export const Hero = ({
  onRunAnalysis,
  isRunning,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter(file => {
      const validTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (url.trim() && productDescription.trim()) {
      ...
      if (uploadedFiles.length > 0) {
        ...
        docsContent = docsContent ? `${docsContent}\n\n[Uploaded documents: ${fileList}]` : `[Uploaded documents: ${fileList}]`;
      }
      onRunAnalysis(projectName, url.trim(), productDescription.trim(), competitors.trim() || undefined, docsContent || undefined, context.trim() || undefined, vision.trim() || undefined, mission.trim() || undefined, values.trim() || undefined, industry || undefined, tone, brandVoice.trim() || undefined);
    }
  };
  ...
  return <section className="w-full max-w-full px-4 sm:px-6 md:px-8 py-8 md:py-16 text-center overflow-x-hidden">
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-3 md:space-y-4">
          <div className="space-y-2 md:space-y-3">
            ...
            <div className="relative">
              <Input type="url" placeholder={t('hero.placeholder.url')} value={url} onChange={e => setUrl(e.target.value)} className="text-sm md:text-base lg:text-lg h-11 md:h-14 pr-10" disabled={isRunning} required />
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                {getUrlValidationIcon()}
              </div>
            </div>
            {urlValidation === 'valid' && url && <p className="text-xs text-green-600 text-left flex items-center gap-1">
                <Check className="w-3 h-3" />
                {t('hero.url.valid')}
              </p>}
            {urlValidation === 'invalid' && url && <p className="text-xs text-destructive text-left flex items-center gap-1">
            ...
            <div className="space-y-1">
              <Textarea placeholder={t('hero.placeholder.product')} value={productDescription} onChange={e => setProductDescription(e.target.value)} className="text-sm md:text-base min-h-[60px] md:min-h-[70px]" disabled={isRunning} required minLength={10} />
              {productDescription.trim().length > 0 && productDescription.trim().length < 10 && <p className="text-xs text-destructive">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <div className="space-y-1">
                <Select value={tone} onValueChange={setTone} disabled={isRunning} required>
                  ...
                  <SelectContent>
                    {TONES.map(t_item => <SelectItem key={t_item.value} value={t_item.value}>
                        {t(t_item.labelKey)}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Button type="submit" size="lg" className="flex-1 h-11 md:h-14 text-sm md:text-base" disabled={isRunning || !tone || !url.trim() || productDescription.trim().length < 10 || urlValidation === 'invalid'}>
                <Sparkles className="mr-2 h-4 w-4 md:h-5 md:w-5" />
                {t('hero.button.run')}
              </Button>
              
              {onLoadDemo && <Button type="button" variant="outline" size="lg" className="h-11 md:h-14 text-sm md:text-base" onClick={onLoadDemo} disabled={isRunning}>
          ...
          <Collapsible open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen}>
            <CollapsibleTrigger asChild>
              <Button type="button" variant="outline" size="sm" className="w-full gap-2" disabled={isRunning}>
            ...
            <CollapsibleContent className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Textarea id="context" placeholder={t('hero.context.placeholder')} value={context} onChange={e => setContext(e.target.value)} disabled={isRunning} className={`min-h-[80px] ${context.length > 5000 ? 'border-destructive' : ''}`} maxLength={5500} />
                ...
                {context.length > 5000 && <Alert variant="destructive">
              ...
              <div className="space-y-2">
                ...
                <Input id="competitors" placeholder={t('hero.competitors.placeholder')} value={competitors} onChange={e => setCompetitors(e.target.value)} disabled={isRunning} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="brandVoice">{t('hero.brandVoice.label')}</Label>
                <Textarea id="brandVoice" placeholder={t('hero.brandVoice.placeholder')} value={brandVoice} onChange={e => setBrandVoice(e.target.value)} disabled={isRunning} className="min-h-[80px]" maxLength={2000} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Textarea id="vision" placeholder={t('hero.vision.placeholder')} value={vision} onChange={e => setVision(e.target.value)} disabled={isRunning} className="min-h-[60px]" />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="mission">{t('hero.mission.label')}</Label>
                  <Textarea id="mission" placeholder={t('hero.mission.placeholder')} value={mission} onChange={e => setMission(e.target.value)} disabled={isRunning} className="min-h-[60px]" />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="values">{t('hero.values.label')}</Label>
                  <Textarea id="values" placeholder={t('hero.values.placeholder')} value={values} onChange={e => setValues(e.target.value)} disabled={isRunning} className="min-h-[60px]" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-3">
                  <div className="border-2 border-dashed rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
                    <input type="file" id="file-upload" multiple accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx" onChange={handleFileUpload} disabled={isRunning || isProcessing} className="hidden" />
                  ...
                  {uploadedFiles.length > 0 && <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        {uploadedFiles.map((file, index) => <div key={index} className="flex items-center gap-2 p-2 bg-muted rounded-md text-sm">
                            <FileText className="w-4 h-4 text-muted-foreground flex-shrink-0" />
                            <span className="flex-1 truncate">{file.name}</span>
                            <Button type="button" variant="ghost" size="sm" onClick={() => removeFile(index)} disabled={isRunning} className="h-6 w-6 p-0">
                              <X className="w-3 h-3" />
                            </Button>
                          </div>)}
                      </div>
                  ...
                  <Textarea id="docs" placeholder={t('hero.docs.placeholder')} value={docs} onChange={e => setDocs(e.target.value)} disabled={isRunning} className="min-h-[80px]" />
                  <p className="text-xs text-muted-foreground">
                    {t('hero.docs.help')}
                  </p>
                </div>
